...
def test_migrate_runtime_data_copies_then_can_remove_identical_source(tmp_path: Path) -> None:
...
def test_migrate_runtime_data_refuses_conflicting_destination(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="Refusing to replace conflicting runtime data"):
