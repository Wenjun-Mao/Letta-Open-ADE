...
def main() -> None:
    ...
    try:
        with httpx.Client(base_url=DEV_UI_BASE_URL, timeout=120.0) as http:
            migration_response = http.get("/api/platform/migration-status")
            migration_response.raise_for_status()
            migration_payload = migration_response.json()
            summary["steps"]["dashboard_status"] = {
                "ok": True,
                "migration_mode": migration_payload.get("migration_mode", ""),
                "platform_api_enabled": bool(migration_payload.get("platform_api_enabled", False)),
                "legacy_api_enabled": bool(migration_payload.get("legacy_api_enabled", False)),
            }
            ...
            options_response = http.get("/api/options")
            ...
            list_agents_response = http.get("/api/agents?limit=200")
            ...
            if not any(str(item.get("id", "") or "") == agent_id for item in agent_items):
                raise RuntimeError("Created agent was not visible from /api/agents")
            ...
            details_response = http.get(f"/api/agents/{agent_id}/details")
            ...
            persistent_response = http.get(f"/api/agents/{agent_id}/persistent_state?limit=80")
            persistent_response.raise_for_status()
            persistent_payload = persistent_response.json()
            raw_prompt_response = http.get(f"/api/agents/{agent_id}/raw_prompt")
            ...
            metadata_response = http.get("/api/platform/metadata/prompts-personas")
            ...
            persona_response = http.patch(
                f"/api/platform/agents/{agent_id}/core-memory/blocks/persona",
            ...
            verify_response = http.get(f"/api/agents/{agent_id}/persistent_state?limit=60")
            ...
            tools_response = http.get(f"/api/platform/tools?limit=200&agent_id={agent_id}")
            ...
            runs_response = http.get("/api/platform/test-runs")
            runs_response.raise_for_status()
            runs_payload = runs_response.json().get("items", [])
            if not any(str(item.get("run_id", "") or "") == run_id for item in runs_payload):
                raise RuntimeError("Created run was not visible in /api/platform/test-runs")

            artifacts_response = http.get(f"/api/platform/test-runs/{run_id}/artifacts")
            ...
            if artifacts:
                ...
                if artifact_id:
                    artifact_read_response = http.get(
                        f"/api/platform/test-runs/{run_id}/artifacts/{artifact_id}?max_lines=120"
                    )
