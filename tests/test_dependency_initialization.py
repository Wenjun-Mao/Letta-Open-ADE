...
from ade_api.dependencies import build_application_services
from ade_api.settings import AdeApiSettings
...
def test_importing_app_does_not_create_runtime_state(tmp_path) -> None:
    ...
    env = {
        **os.environ,
        "ADE_API_RUNTIME_DATA_DIR": str(runtime_dir),
        "ADE_API_PERSONA_DB_PATH": str(persona_db),
        "ADE_API_PERSONA_SEED_JSONL_PATH": str(tmp_path / "missing-seed.jsonl"),
    }

    result = subprocess.run(
        [sys.executable, "-c", "import ade_api.app"],
        cwd=os.getcwd(),
        env=env,
        capture_output=True,
        text=True,
