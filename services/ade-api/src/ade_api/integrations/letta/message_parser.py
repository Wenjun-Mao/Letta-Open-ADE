...
def _parse_message_content(msg) -> dict[str, Any]:
    ...
    if m_type == "user_message":
        ...
        name = (
            getattr(tool_call, "name", "unknown_tool") if tool_call else "unknown_tool"
        )
...
def chat(client: Letta, agent_id: str, **kwargs) -> dict:
    ...
    response = client.agents.messages.create(agent_id=agent_id, **kwargs)
    ...
    return {
        ...
        "sequence": steps,
        "memory_diff": {"old": old_mem, "new": new_mem},
...
def pretty_print_messages(chat_result: dict, focus_block: str = "human"):
    ...
    for i, step in enumerate(chat_result["sequence"], 1):
        ...
        if step["type"] == "user":
            ...
            print(
                f"{step_str} 🔧 TOOL CALLED [{step.get('name')}]:\n  Arguments: {step.get('arguments')}"
            )
        elif step["type"] == "tool_return":
            print(
                f"{step_str} ✅ TOOL RETURN [{step.get('status')}]: {step.get('content')}"
            )
        else:
