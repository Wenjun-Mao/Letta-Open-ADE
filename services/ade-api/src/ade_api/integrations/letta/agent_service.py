...
from ade_api.integrations.letta.message_parser import chat
...
class LettaAgentService:
    """ADE's typed boundary for persistent Letta agent operations."""
    ...
    def _message_create_params(self, client: Letta | None = None) -> set[str]:
        ...
        return set(
            inspect.signature(target_client.agents.messages.create).parameters.keys()
        )
    ...
    def _clamp_timeout_seconds(value: float | None) -> float:
        ...
        return max(
            MIN_RUNTIME_TIMEOUT_SECONDS, min(MAX_RUNTIME_TIMEOUT_SECONDS, float(value))
        )
    ...
    def _runtime_client(
        self, *, timeout_seconds: float | None, retry_count: int | None
    ) -> Letta:
    ...
    def _is_context_limit_error(exc: Exception) -> bool:
        text = str(exc).lower()
        return (
            "context size has been exceeded" in text or "maximum context length" in text
        )

    def capabilities(self) -> dict[str, Any]:
        message_params = self._message_create_params()
        update_params = set(
            inspect.signature(self._client.agents.update).parameters.keys()
        )
        block_update_params = set(
            inspect.signature(self._client.agents.blocks.update).parameters.keys()
        )
        ...
        *,
        ...
        if override_model:
            if "override_model" in message_params:
                ...
                raise ValueError(
                    "The active Letta SDK does not support request-time model override."
                )
        ...
        if override_system:
            if "override_system" in message_params:
                ...
                raise ValueError(
                    "The active Letta SDK does not support request-time system override."
                )
    ...
    def update_core_memory_block(
        self, *, agent_id: str, block_label: str, value: str
    ) -> dict[str, Any]:
        before = self._client.agents.blocks.retrieve(
            agent_id=agent_id, block_label=block_label
        )
        updated = self._client.agents.blocks.update(
            agent_id=agent_id, block_label=block_label, value=value
        )
