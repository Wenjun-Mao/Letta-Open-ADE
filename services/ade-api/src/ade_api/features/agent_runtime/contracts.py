...
class AgentStudioOptionsResponse(BaseModel):
    runtime: Literal["ade_native"] = "ade_native"
...
class CreateEvaluationSessionRequest(StrictRequest):
    """Provision one isolated evaluation conversation and its default resources."""

    idempotency_key: str = Field(min_length=1, max_length=200)
    title: str = Field(default="Evaluation conversation", min_length=1, max_length=120)
    agent_definition_id: str | None = Field(default=None, min_length=1, max_length=100)
    model_key: str | None = Field(default=None, min_length=1, max_length=300)
    reviewer_model_key: str | None = Field(default=None, min_length=1, max_length=300)
    embedding_model_key: str | None = Field(default=None, min_length=1, max_length=300)
    prompt_key: str = Field(default="chat_v20260516", min_length=1, max_length=128)
    persona_key: str = Field(default="chat_linxiaotang", min_length=1, max_length=128)
    tool_names: list[Literal["search_memory", "get_weather"]] = Field(
        default_factory=lambda: ["search_memory"], max_length=2
    ...
    memory_subject_id: str | None = Field(default=None, min_length=1, max_length=100)
    subject_external_key: str | None = Field(default=None, min_length=1, max_length=200)
    subject_display_name: str = Field(default="", max_length=200)

    @field_validator("idempotency_key", "title")
    @classmethod
    def _reject_blank_evaluation_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("value must not be blank")
        return value.strip()
    ...
    @field_validator("tool_names")
    @classmethod
    def _dedupe_evaluation_tools(cls, value: list[str]) -> list[str]:
        if len(value) != len(set(value)):
            raise ValueError("tool_names must not contain duplicates")
        return value

    @model_validator(mode="after")
    def _require_unambiguous_definition_and_subject(
        self,
    ) -> CreateEvaluationSessionRequest:
        routes = (
            self.model_key,
            self.reviewer_model_key,
            self.embedding_model_key,
        )
        if self.agent_definition_id is None and any(route is None for route in routes):
            raise ValueError(
                "model_key, reviewer_model_key, and embedding_model_key are required "
                "when agent_definition_id is not supplied"
            )
        if self.agent_definition_id is not None and any(
            route is not None for route in routes
        ):
            raise ValueError(
                "model route keys cannot be combined with agent_definition_id"
            )
        ...
        if self.agent_definition_id is not None and (
            definition_configuration & self.model_fields_set
        ):
            raise ValueError(
                "definition configuration cannot be combined with agent_definition_id"
            )
        subject_configuration = {"subject_external_key", "subject_display_name"}
        if self.memory_subject_id is not None and (
            subject_configuration & self.model_fields_set
        ):
            raise ValueError(
                "subject configuration cannot be combined with memory_subject_id"
            )
        return self

    def definition_request(
        self, *, definition_key: str
    ) -> CreateAgentDefinitionRequest:
        if self.agent_definition_id is not None:
            raise ValueError("existing definitions do not need a creation request")
        assert self.model_key is not None
        assert self.reviewer_model_key is not None
    ...
    def subject_request(self, *, external_key: str) -> CreateMemorySubjectRequest:
        if self.memory_subject_id is not None:
            raise ValueError("existing subjects do not need a creation request")
        return CreateMemorySubjectRequest(
            external_key=self.subject_external_key or external_key,
            display_name=self.subject_display_name,
        )
...
class EvaluationSessionResponse(AgentStudioSessionResponse):
    """The isolated resources created or bound for an evaluation conversation."""


class EvaluationSessionStateResponse(BaseModel):
    conversation: ConversationStateResponse
    memory_subject: MemorySubjectResponse
    memories: SubjectMemoriesResponse
    latest_run: RunResponse | None = None


class EvaluationSessionPurgeResponse(BaseModel):
    conversation_id: str
