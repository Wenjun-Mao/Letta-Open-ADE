...
def validate_definition_execution(
    ...
    *,
    ...
    expected_agent_bundle: Mapping[str, Any] | None = None,
    source_clean: bool | None = None,
) -> None:
    if mode == "release":
        ...
        if expected_agent_bundle is None or {
            "prompt_key": definition.get("prompt_key"),
            "persona_key": definition.get("persona_key"),
            "tool_names": definition.get("tool_names"),
        } != {
            "prompt_key": expected_agent_bundle.get("prompt_key"),
            "persona_key": expected_agent_bundle.get("persona_key"),
            "tool_names": expected_agent_bundle.get("tool_names"),
        }:
            raise UnqualifiedDeployment(
                "Agent definition bundle is outside the qualified Agent Studio contract"
            )
