...
from .errors import AgentRuntimeError, RuntimeNotReady
            ...
            async with self.database.translated_errors():
                async with self.engine.connect() as connection:
                    snapshot = await WorkerInstanceRepository(
                        ...
                        freshness_seconds=(
                            self.settings.agent_runtime_worker_stale_seconds
                        ),
                    )
...
class WorkerPresence:
            ...
            await WorkerInstanceRepository(connection).register(
                {
                    ...
                    "worker_id": self.settings.agent_runtime_worker_id,
                    "state": "ready",
    ...
    async def heartbeat_forever(self, stop: asyncio.Event) -> None:
        interval = self.settings.agent_runtime_worker_heartbeat_seconds
...
def _not_ready_health(
    *,
    ...
    return {
        ...
        "freshness_seconds": float(settings.agent_runtime_worker_stale_seconds),
