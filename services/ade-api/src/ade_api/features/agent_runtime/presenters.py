...
def subject_response(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "memory_generation": int(row.get("memory_generation", 1)),
...
def run_response(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "accepted_memory_generation": int(row.get("accepted_memory_generation", 1)),
...
def memory_fact_response(
    ...
    return {
        ...
        "value": row["value"],
        "status": row["status"],
        "assertion_schema_version": int(row.get("assertion_schema_version", 1)),
        "version": int(row["version"]),
        "revisions": revisions,
