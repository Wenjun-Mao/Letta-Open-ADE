...
class AttemptController:
    ...
    async def heartbeat(
        ...
        interval = min(
            self.settings.agent_runtime_heartbeat_seconds,
            max(1, self.settings.agent_runtime_lease_seconds // 3),
        )
        while not stop.is_set():
            ...
            async with self.engine.begin() as connection:
                owned = await ConversationLeaseRepository(connection).heartbeat(
                    claim.lease_token,
                    utc_now()
                    + timedelta(seconds=self.settings.agent_runtime_lease_seconds),
                )
            if not owned:
                lease_lost.set()
                return
