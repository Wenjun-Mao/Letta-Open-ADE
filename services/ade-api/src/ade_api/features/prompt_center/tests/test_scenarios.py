...
def test_create_comment_persona_infers_comment_scenario_from_key(tmp_path) -> None:
    ...
    assert (
        record["source_path"] == "data/runtime/personas/personas.sqlite3#comment_test"
    )
...
def test_create_chat_prompt_infers_chat_scenario_from_key(tmp_path) -> None:
    ...
    assert (
        tmp_path / "content" / "prompts" / "system" / "chat" / "chat_test.py"
    ).exists()
...
def test_create_label_prompt_infers_label_scenario_from_key(tmp_path) -> None:
    ...
    assert (
        tmp_path / "content" / "prompts" / "system" / "label" / "label_test.py"
    ).exists()
...
def test_comment_template_lifecycle_respects_scenario_filters(tmp_path) -> None:
    ...
    registry.create_template(
        "persona", key="comment_test", content="Comment persona body"
    )
    ...
    archived_list = registry.list_templates(
        "persona", include_archived=True, scenario="comment"
    )
    ...
    remaining = registry.list_templates(
        "persona", include_archived=True, scenario="comment"
    )
    assert remaining == []
...
def test_custom_templates_can_omit_label_and_description_metadata(tmp_path) -> None:
    ...
    source_path = (
        tmp_path / "content" / "prompts" / "system" / "chat" / "chat_optional_meta.py"
    )
...
def test_label_prompt_parses_output_schema_metadata(tmp_path) -> None:
    prompt_path = (
        tmp_path / "content" / "prompts" / "system" / "label" / "label_schema.py"
    )
