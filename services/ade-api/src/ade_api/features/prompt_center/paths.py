...
class PromptPersonaPaths:
    ...
    def from_project_root(cls, project_root: Path) -> "PromptPersonaPaths":
        ...
        return cls(
            ...
            prompt_dir=resolved_project_root / "content" / "prompts" / "system",
            prompt_archive_dir=resolved_project_root
            / "content"
            / "prompts"
            / "system"
            / "archive",
            persona_dir=resolved_project_root / "content" / "personas",
            persona_archive_dir=resolved_project_root
            / "content"
            / "personas"
            / "archive",
        )
    ...
    def file_path_for_create(
        self, kind: TemplateKind, key: str, scenario: ScenarioKind, archived: bool
    ) -> Path:
        ...
        *,
        ...
        for file_path in self.iter_template_files(kind=kind, archived=archived):
            if file_path.stem != key:
                continue
            inferred_scenario = self.infer_scenario_from_path(
