...
class PersonaExchangeRegistry(Protocol):
    ...
    def archive_persona(
        self, key: str, scenario: str | None = None
    ) -> dict[str, Any]: ...
...
def import_personas_jsonl(
    ...
    *,
    ...
    for line_number, raw_line in enumerate(
        ...
        if is_archived:
            registry.archive_persona(
                str(record["key"]), scenario=record.get("scenario")
            )
        counts["created"] += 1
    return counts
...
def export_personas_jsonl(
    ...
    *,
    ...
    records = registry.list_personas(
        include_archived=include_archived, scenario=scenario
    )
