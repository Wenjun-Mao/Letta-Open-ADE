...
from ade_api.features.prompt_center.codec import parse_template_file, render_source
from ade_api.features.prompt_center.paths import PromptPersonaPaths
from ade_api.features.prompt_center.personas.sqlite import PersonaSqliteRegistry
from ade_api.features.prompt_center.types import (
    KEY_PATTERN,
    KNOWN_SCENARIOS,
    RegistryError,
    ScenarioKind,
...
class PromptPersonaRegistry:
    ...
    def list_templates(
        ...
        records = self._list_from_dir(
            kind=kind, archived=False, scenario=resolved_scenario
        )
        if include_archived:
            records.extend(
                self._list_from_dir(
                    kind=kind, archived=True, scenario=resolved_scenario
                )
            )
        records.sort(
            key=lambda item: (
    ...
    def get_template(
        ...
        *,
        ...
        if kind == "persona":
            return self.persona_registry.get_persona(
                key, archived=archived, scenario=scenario
            )
    ...
    def create_template(
        ...
        *,
        ...
        normalized = self._normalize_key(key)
        resolved_scenario = self._normalize_scenario(
            scenario, allow_none=True
        ) or self._infer_scenario_from_key(normalized)
        self._ensure_supported_persona_scenario(kind, resolved_scenario)
        self._validate_key_scenario(normalized, resolved_scenario)

        if self.paths.find_template_path(
            kind=kind, key=normalized, archived=False, scenario=None
        ).exists():
            raise RegistryError(f"{kind} '{normalized}' already exists")
        if self.paths.find_template_path(
            kind=kind, key=normalized, archived=True, scenario=None
        ).exists():
            raise RegistryError(
                f"{kind} '{normalized}' already exists in archive; restore it instead"
            )
    ...
    def archive_template(
        self, kind: TemplateKind, key: str, scenario: ScenarioKind | None = None
    ) -> dict[str, Any]:
        if kind == "persona":
    ...
    @staticmethod
    def _is_unsupported_persona_scenario(
        kind: TemplateKind, scenario: ScenarioKind | None
    ) -> bool:
        return kind == "persona" and scenario == "label"

    def _ensure_supported_persona_scenario(
        self, kind: TemplateKind, scenario: ScenarioKind | None
    ) -> None:
    ...
    def _list_from_dir(
        ...
        for file_path in self.paths.iter_template_files(kind=kind, archived=archived):
            try:
                parsed = self._parse_template_file(
                    kind=kind, path=file_path, archived=archived
                )
    ...
    def _parse_template_file(
        self, kind: TemplateKind, path: Path, archived: bool
    ) -> dict[str, Any]:
        return parse_template_file(
            kind=kind, path=path, archived=archived, paths=self.paths
        )
