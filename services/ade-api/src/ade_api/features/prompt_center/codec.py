...
from ade_api.features.prompt_center.defaults import (
...
def parse_template_file(
    *,
    ...
    module_doc_preview = (
        module_doc.strip().splitlines()[0].strip() if module_doc.strip() else ""
    )
    ...
    if (
        not description
        and module_doc_preview
        and module_doc_preview != GENERATED_MODULE_DOC
    ):
    ...
    if kind == "prompt":
        output_schema = (
            str(assignments.get(OUTPUT_SCHEMA_VAR, "") or "").strip() or None
        )
    stat = path.stat()
    return {
        "kind": kind,
...
def render_source(
    *,
    ...
    if resolved_description:
        lines.append(
            f"{META_DESCRIPTION} = {json.dumps(resolved_description, ensure_ascii=False)}"
        )
    lines.extend(
        [
