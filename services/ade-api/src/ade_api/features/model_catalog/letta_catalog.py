...
def resolve_letta_catalog_handles(client: Letta) -> tuple[set[str], set[str]]:
    ...
    try:
        for model in list(client.models.list()):
            ...
            if not handle:
                continue
            if looks_like_embedding_handle(handle) or model_type in {
                "embedding",
                "embeddings",
            }:
                discovered_embedding_handles.add(handle)
