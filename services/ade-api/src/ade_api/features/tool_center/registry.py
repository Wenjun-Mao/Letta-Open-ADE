...
class CustomToolRegistry:
    ...
    def __init__(self, project_root: Path):
        ...
        self.base_dir = self.project_root / "content" / "custom-tools"
    ...
    def list_tools(
        self, *, include_archived: bool = False, include_source: bool = False
    ) -> list[dict[str, Any]]:
        manifest = self._load_manifest()
        output: list[dict[str, Any]] = []
        tools = (
            manifest.get("tools", {})
            if isinstance(manifest.get("tools", {}), dict)
            else {}
        )
        ...
        for slug, payload in tools.items():
            ...
            item = self._record_for(
                slug=slug, payload=payload, include_source=include_source
            )
            output.append(item)
    ...
    def get_tool(
        ...
        return self._record_for(
            slug=normalized, payload=payload, include_source=include_source
        )
    ...
    def archive_tool(self, slug: str) -> dict[str, Any]:
        ...
        if not active_path.exists():
            raise ToolRegistryError(
                f"Source file for tool '{normalized}' was not found"
            )
        if archived_path.exists():
            raise ToolRegistryError(
                f"Archived source already exists for '{normalized}'"
            )
    ...
    def restore_tool(
        ...
        *,
        ...
        if not archived_path.exists():
            raise ToolRegistryError(
                f"Archived source file for '{normalized}' was not found"
            )
        if active_path.exists():
            raise ToolRegistryError(
                f"Active source file already exists for '{normalized}'"
            )
    ...
    def purge_tool(self, slug: str) -> None:
        ...
        if not bool(payload.get("archived", False)):
            raise ToolRegistryError(
                f"Tool '{normalized}' must be archived before purge"
            )
    ...
    def _record_for(
        self, *, slug: str, payload: dict[str, Any], include_source: bool
    ) -> dict[str, Any]:
        archived = bool(payload.get("archived", False))
    ...
    def _load_manifest(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise ToolRegistryError(
                f"Failed to parse tool registry manifest: {exc}"
            ) from exc
