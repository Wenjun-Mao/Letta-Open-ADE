...
from ade_api.platform.constants import MANAGED_TOOL_TAG


def managed_tool_tags(extra_tags: list[str] | None = None) -> list[str]:
    """Ensure every persisted Tool Center tool remains identifiable at runtime."""
...
def as_tool_center_item(
    *,
    ...
    """Merge persisted tool metadata with the current Letta runtime record."""
    if managed_entry:
        return {
            "slug": str(managed_entry.get("slug", "") or ""),
            "tool_id": str(managed_entry.get("tool_id", "") or ""),
            "name": str(
                (remote_tool or {}).get("name", managed_entry.get("name", "")) or ""
            ),
            "description": str(
                (remote_tool or {}).get(
                    "description", managed_entry.get("description", "")
                )
                or ""
            ),
            "tool_type": str(
                (remote_tool or {}).get(
                    "tool_type", managed_entry.get("tool_type", "custom")
                )
                or "custom"
            ),
            "source_type": str(
                (remote_tool or {}).get(
                    "source_type", managed_entry.get("source_type", "python")
                )
                or "python"
            ),
            "tags": [
                str(tag)
                for tag in (
                    (remote_tool or {}).get("tags", managed_entry.get("tags", [])) or []
                )
                if str(tag).strip()
            ],
            "managed": True,
            ...
            "source_code": str(managed_entry.get("source_code", "") or "")
            if include_source
            else None,
            "created_at": str(
                (remote_tool or {}).get(
                    "created_at", managed_entry.get("created_at", "")
                )
                or ""
            ),
            "last_updated_at": str(
                (remote_tool or {}).get(
                    "last_updated_at", managed_entry.get("updated_at", "")
                )
                or ""
            ),
