...
from ade_api.features.comment_lab.contracts import (
    ApiCommentingGenerateResponse,
    CommentingGenerateRequest,
)
from ade_api.platform.openapi_metadata import TAG_COMMENT_LAB
...
def api_commenting_generate(
    ...
    if request.include_diagnostics and principal.role < AdeRole.ADMIN:
        raise HTTPException(
            status_code=403, detail="Raw provider diagnostics require the admin role."
        )

    resolved_scenario = normalize_scenario(request.scenario, default="comment")
    if resolved_scenario != "comment":
        raise HTTPException(
            status_code=400, detail="scenario must be 'comment' for this endpoint"
        )
    ...
    prompt_map = prompt_content_map(prompt_registry, "comment")
    persona_map = persona_content_map(prompt_registry, "comment")
    ...
    try:
        generation_result = commenting_service.generate_comment(
            ...
            temperature=(
                ...
                else _scenario_sampling_default(
                    model_selection, "comment_lab", "temperature"
                )
            ),
            top_p=(
                request.top_p
    ...
    if not content:
        raise HTTPException(
            status_code=400, detail="Comment provider returned empty content"
        )

    runtime_defaults = commenting_runtime_defaults(commenting_service)
    ...
    if not isinstance(raw_request, dict):
        raw_request = {}
    selected_attempt = (
        str(generation_result.get("selected_attempt", "") or "").strip() or "unknown"
    )
    ...
    response = {
        ...
        "max_tokens": int(
            generation_result.get("max_tokens", runtime_defaults.max_tokens)
        ),
        "timeout_seconds": float(
            generation_result.get("timeout_seconds", runtime_defaults.timeout_seconds)
        ),
        "task_shape": str(
            generation_result.get("task_shape", runtime_defaults.task_shape)
        ),
        "cache_prompt": bool(
            generation_result.get("cache_prompt", runtime_defaults.cache_prompt)
        ),
        "enable_thinking": bool(generation_result.get("enable_thinking", False)),
        "temperature": float(
            generation_result.get("temperature", runtime_defaults.temperature)
        ),
        "top_p": float(generation_result.get("top_p", runtime_defaults.top_p)),
        "top_k": generation_result.get("top_k", runtime_defaults.top_k),
        "content_source": str(generation_result.get("content_source", "") or "")
        or None,
