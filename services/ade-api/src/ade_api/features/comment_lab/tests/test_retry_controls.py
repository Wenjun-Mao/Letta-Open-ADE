...
from ade_api.features.comment_lab.service import (
...
def test_chat_completions_url_supports_v1_and_v3_bases() -> None:
    ...
    assert CommentingService._chat_completions_url(
...
def test_retry_count_controls_total_attempts(
    monkeypatch, retry_count: int, expected_attempts: int
) -> None:
    service = _build_service()
    attempts = {"count": 0}
...
def test_structured_output_fallback_stays_separate_from_retry_policy(
    monkeypatch,
) -> None:
    service = _build_service()
    ...
    def fake_post(payload, *, base_url, api_key, timeout_seconds, retry_count):
        ...
        return {
            "choices": [
                {
                    "message": {
                        "content": json.dumps({"comment": "Fallback succeeded."})
                    },
                    "finish_reason": "stop",
                }
            ],
    ...
    monkeypatch.setattr(
        ...
        lambda payload, *, base_url, api_key, timeout_seconds, retry_count: {
            "choices": [
                {
                    "message": {
                        "content": "<think>private reasoning</think>Public reply"
                    },
                    "finish_reason": "stop",
                }
            ],
...
def test_generate_comment_treats_zero_max_tokens_as_omitted_provider_field(
    monkeypatch,
) -> None:
    service = _build_service()
    captured: dict[str, object] = {}
...
def test_generate_comment_sends_vllm_thinking_toggle_and_reads_reasoning_field(
    monkeypatch,
) -> None:
    service = _build_service()
    captured: dict[str, object] = {}
