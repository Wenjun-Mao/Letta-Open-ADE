...
from ade_api.features.label_lab.service import LabelingService, LabelingValidationError
...
def test_generate_labels_json_schema_sends_llama_server_response_format(
    monkeypatch,
) -> None:
    service = _build_service()
...
def test_generate_labels_uses_repair_attempt_after_validation_failure(
    monkeypatch,
) -> None:
    service = _build_service()
...
def test_generate_labels_raises_validation_error_after_repair_failure(
    monkeypatch,
) -> None:
    service = _build_service()
    ...
    with pytest.raises(
        LabelingValidationError, match="invalid structured output"
    ) as exc_info:
        service.generate_labels(
            base_url="http://127.0.0.1:2234/v1",
            api_key="local-token",
