...
from ade_api.platform.auth import AdePrincipal, AdeRole
from ade_api.features.label_lab import api as label_lab
from ade_api.features.label_lab.contracts import LabelingGenerateRequest
...
def test_labeling_generate_uses_model_key_and_selected_source_connection(
    ...
    monkeypatch.setattr(label_lab, "ensure_ade_api_enabled", lambda: None)
    monkeypatch.setattr(
        label_lab,
    ...
    monkeypatch.setattr(
        label_lab,
        "prompt_record_map",
        lambda _registry, scenario=None: {
    ...
    labeling_service = SimpleNamespace(generate_labels=fake_generate_labels)

    payload = label_lab.api_labeling_generate(
...
def test_labeling_generate_returns_validation_errors_on_failure(monkeypatch) -> None:
    ...
    labeling_service = SimpleNamespace(
        generate_labels=lambda **kwargs: (_ for _ in ()).throw(
    ...
    with pytest.raises(HTTPException) as exc_info:
        label_lab.api_labeling_generate(
            LabelingGenerateRequest(
                input="Messi scored.",
                prompt_key="label_generic_entities_v1",
                model_key="local_llama_server::gemma4",
