...
class AgentLifecycleRegistry:
    """File-backed archive state for agents managed by ADE API."""

    def __init__(self, project_root: Path, *, base_dir: Path | None = None):
        self.project_root = Path(project_root)
        self.base_dir = Path(
            base_dir or self.project_root / "data" / "runtime" / "agent-lifecycle"
        ).resolve()
    ...
    def archive_agent(
        ...
        if isinstance(current, dict) and bool(current.get("archived", False)):
            raise AgentLifecycleRegistryError(
                f"Agent '{resolved_agent_id}' is already archived"
            )
    ...
    def restore_agent(self, agent_id: str) -> dict[str, Any]:
        ...
        if not isinstance(payload, dict) or not bool(payload.get("archived", False)):
            raise AgentLifecycleRegistryError(
                f"Agent '{resolved_agent_id}' is not archived"
            )
    ...
    def _read_manifest(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise AgentLifecycleRegistryError(
                f"Failed to read lifecycle registry: {exc}"
            ) from exc

        if not isinstance(payload, dict):
            raise AgentLifecycleRegistryError(
                "Lifecycle registry root must be an object"
            )
