...
import ade_api.features.agent_runtime.turn_execution as turn_execution_module
...
def test_deepseek_required_search_uses_auto_and_replays_reasoning_with_tool_result() -> (
    ...
    evidence = NaturalAttemptEvidence(
        run_id="synthetic-run", attempt=1, policy_binding="natural-user-assertions-v4-b"
    )
    result = asyncio.run(
...
def test_weather_fixture_registry_is_not_available_without_evaluation() -> None:
    with pytest.raises(RuntimeValidationError, match="Unknown or unavailable"):
        curated_tools(("get_weather",))


def test_search_memory_arguments_cannot_select_another_subject() -> None:
    transport = _Transport(
        [
            {
                "choices": [
                            ...
                            "tool_calls": [
                                {
                                    ...
                                    "function": {
                                        ...
                                        "arguments": (
                                            '{"query":"Rocky",'
                                            '"memory_subject_id":"other-subject"}'
                                        ),
                                    },
                                }
                            ]
        ...
        asyncio.run(
            ConversationExecutor(transport).execute(
                ...
                tools=curated_tools(
                    ...
                    additional_tools=evaluation_tool_registry(),
                ),
                tool_requirement=ToolRequirement(
                    tool_name="get_weather", capability="weather.current_lookup"
                ),
