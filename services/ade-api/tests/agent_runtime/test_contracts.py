...
def test_evaluation_session_contract_provisions_defaults_or_binds_fixtures() -> None:
    request = CreateEvaluationSessionRequest(
        ...
        subject_external_key="fixture:customer-1",
    )

    definition = request.definition_request(definition_key="evaluation_123")
    subject = request.subject_request(external_key="evaluation:generated")

    assert definition.tool_names == ["search_memory", "get_weather"]
    assert subject.external_key == "fixture:customer-1"

    fixture = CreateEvaluationSessionRequest(
        idempotency_key="evaluation-2",
        agent_definition_id="definition-version-1",
        memory_subject_id="subject-1",
    )
    assert fixture.agent_definition_id == "definition-version-1"

    with pytest.raises(ValidationError, match="model_key"):
        CreateEvaluationSessionRequest(idempotency_key="evaluation-3")
    with pytest.raises(ValidationError, match="cannot be combined"):
        CreateEvaluationSessionRequest(
            idempotency_key="evaluation-4",
            agent_definition_id="definition-version-1",
            model_key="dgx_vllm::qwen",
        )
    with pytest.raises(ValidationError, match="definition configuration"):
        CreateEvaluationSessionRequest(
            idempotency_key="evaluation-4b",
            agent_definition_id="definition-version-1",
            prompt_key="other_prompt",
        )
    with pytest.raises(ValidationError, match="subject configuration"):
        CreateEvaluationSessionRequest(
            idempotency_key="evaluation-4c",
            agent_definition_id="definition-version-1",
            memory_subject_id="subject-1",
            subject_display_name="Ignored otherwise",
        )
    with pytest.raises(ValidationError, match="must not contain duplicates"):
        CreateEvaluationSessionRequest(
            idempotency_key="evaluation-5",
            model_key="dgx_vllm::qwen",
            reviewer_model_key="dgx_vllm::qwen-reviewer",
            embedding_model_key="dgx_embedding::qwen",
