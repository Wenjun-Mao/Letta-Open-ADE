...
def test_load_model_profiles_resolves_relative_path_from_repository_root(
    monkeypatch,
    tmp_path,
) -> None:
    profiles_path = tmp_path / "config" / "model-router" / "model-profiles.json"
    profiles_path.parent.mkdir(parents=True)
    profiles_path.write_text(
        json.dumps({"dgx_vllm::gemma4-31b-nvfp4": _profile_payload()}),
        encoding="utf-8",
    )
    monkeypatch.setenv("ADE_REPOSITORY_ROOT", str(tmp_path))

    profiles = load_model_profiles("config/model-router/model-profiles.json")

    assert "dgx_vllm::gemma4-31b-nvfp4" in profiles
...
def test_load_model_profiles_accepts_qwen_vllm_sampling_defaults(tmp_path) -> None:
    ...
    assert (
        profile.effective_defaults_for("comment_lab").as_payload()["presence_penalty"]
        == 1.5
    )
    assert (
        profile.effective_defaults_for("label_lab").as_payload()["repetition_penalty"]
        == 1.0
    )
