...
import ade_api.features.model_catalog.letta_catalog as letta_catalog
...
def test_runtime_options_use_router_catalog_and_letta_intersection(monkeypatch) -> None:
    ...
    dependencies = {
        "model_router_client": _FakeRouterClient(),
        "letta_client": object(),
        "force_refresh": True,
    }

    chat_options, _ = runtime_options("chat", **dependencies)
    comment_options, _ = runtime_options("comment", **dependencies)
    label_options, _ = runtime_options("label", **dependencies)
...
def test_resolve_comment_selection_uses_router_base_url_and_key(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        letta_catalog,
        "resolve_letta_catalog_handles",
        lambda _client: ({"openai-proxy/local_llama_server::gemma4"}, set()),
    )

    selection = resolve_comment_model_selection(
        model_key="local_llama_server::gemma4",
