...
- Persistent agent behavior stays with the ADE-owned Agent Studio runtime;
  stateless labs use Model Router for provider requests.
...
The current steady-state system is defined by
[ADR 0019](adr/0019-ade-steady-state-runtime.md). This record does not describe
the current Compose graph or release procedure.
