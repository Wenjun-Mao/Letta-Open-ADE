...
`never` approval, disabled web search, and ChatGPT-only login setting. Sixteen
...
| Observed inherited MCP control | Under the same sanitized environment, `mcp list --json` showed enabled `cua_repl`, `node_repl`, and `openaiDeveloperDocs` despite the feature flags. Per-server `enabled=false` alone failed config parsing for the two host-injected stdio servers because that invocation layer lacked their transport. Version-pinned invocation overrides provide an inert `/usr/bin/false` transport and disable those two; the HTTP docs server needs only `enabled=false`. The spike now fails closed if *any* CLI-listed MCP server remains enabled and verifies every MCP entry in app-server's own `config/read` is disabled. The no-generation installed probe listed all five inherited servers as disabled; global configuration was not edited. This validates the observed MCP layer, not all built-in or plugin tool exposure. |
...
| Testable isolation unknown | App-server help has no `--ignore-user-config`. `mcp_servers={}` on the invocation did not clear inherited MCP entries; a fresh isolated `CODEX_HOME` reported “Not logged in.” The candidate flags disabled `shell_tool`, but `features.unified_exec=false` still reported `unified_exec=true` in `features list`. Installed plugin metadata also reports enabled plugins, and the MCP listing is not an exhaustive app-server tool catalog. These observations do not prove a command or plugin tool remains callable, but neither do they prove that only ADE's dynamic tool is exposed. A complete effective tool inventory or independently enforced process boundary remains necessary. |
