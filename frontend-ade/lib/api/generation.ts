...
export function listLabelSchemas(includeArchived = false, options?: ApiRequestOptions) {
  ...
  return requestJson<{ total: number; include_archived: boolean; items: LabelSchemaRecord[] }>(
    `/api/v1/platform/schema-center/label-schemas?${params.toString()}`,
    options,
  );
}

export function createLabelSchema(payload: { key: string; label?: string; description?: string; schema: Record<string, unknown> }) {
  return requestJson<LabelSchemaRecord>("/api/v1/platform/schema-center/label-schemas", { method: "POST", body: payload });
}
...
export function updateLabelSchema(
  ...
  return requestJson<LabelSchemaRecord>(`/api/v1/platform/schema-center/label-schemas/${key}`, { method: "PATCH", body: payload });
}

export function archiveLabelSchema(key: string) {
  return requestJson<LabelSchemaRecord>(`/api/v1/platform/schema-center/label-schemas/${key}/archive`, { method: "POST" });
}

export function restoreLabelSchema(key: string) {
  return requestJson<LabelSchemaRecord>(`/api/v1/platform/schema-center/label-schemas/${key}/restore`, { method: "POST" });
}

export function purgeLabelSchema(key: string) {
  return requestJson<{ ok: boolean; key: string; kind: string }>(`/api/v1/platform/schema-center/label-schemas/${key}/purge`, { method: "DELETE" });
}
