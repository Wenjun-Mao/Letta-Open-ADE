...
ADE_API_BASE_URL = os.getenv("ADE_API_BASE_URL", "http://127.0.0.1:8000")
...
def _resolve_runtime_defaults(http: httpx.Client) -> dict[str, str]:
    response = http.get("/api/v2/model-catalog/options", params={"scenario": "chat"})
    ...
    if not embedding and any(
        str(item.get("key", "") or "") == DEFAULT_EMBEDDING_HANDLE
        for item in embeddings
    ):
...
def main() -> None:
    ...
    try:
        with httpx.Client(
            ...
            resolved_model = str(
                runtime_defaults.get("model", "") or DEFAULT_TEST_MODEL_HANDLE
            )
            resolved_prompt_key = str(
                runtime_defaults.get("prompt_key", "") or DEFAULT_PROMPT_KEY
            )
            ...
            scenario_guard = http.post(
                "/api/v2/agent-studio/agents", json=guard_payload
            )
            if scenario_guard.status_code != 400:
                raise RuntimeError(
                    "Expected /api/v2/agent-studio/agents to reject non-chat scenario, "
            ...
            if not sequence:
                raise RuntimeError("ADE runtime message returned an empty sequence")
            summary["steps"]["runtime_message"] = {
                "ok": True,
                "total_steps": int(
                    message_payload.get("result", {}).get("total_steps", 0)
                ),
            }
            ...
            summary["steps"]["tool_test_invoke"] = {
                ...
                "tool_return_count": int(
                    tool_probe_payload.get("tool_return_count", 0)
                ),
            }

            system_text = f"E2E system update at {int(time.time())}"
            system_response = http.patch(
                f"/api/v2/agent-studio/agents/{agent_id}/system-prompt",
            ...
            if system_payload.get("system_after") != system_text:
                raise RuntimeError(
                    "System prompt update did not persist expected value"
                )
            ...
            model_response = http.patch(
                f"/api/v2/agent-studio/agents/{agent_id}/model",
            ...
            if block_payload.get("value_after") != human_value:
                raise RuntimeError(
                    "Core-memory block update did not persist expected value"
                )
            ...
            if not revision_items:
                raise RuntimeError(
                    "Prompt/persona revision history returned no records"
                )

            revision_fields = {
                str(item.get("field", "") or "") for item in revision_items
            }
            ...
            tools_response = http.get(
                ...
                params={"agent_id": agent_id, "limit": 500},
            )
            tools_response.raise_for_status()
            attached_tools = [
                item
                for item in tools_response.json().get("items", [])
                if bool(item.get("attached_to_agent", False))
            ]
            if attached_tools:
                tool_id = str(attached_tools[0].get("id", "") or "")
                if tool_id:
        ...
        summary["ok"] = True
        summary["detail"] = "all ADE API checks passed"
