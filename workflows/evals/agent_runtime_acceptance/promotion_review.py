...
def review_promotion(
    *,
    ...
    _require_equal(
        proposal.get("kind"),
        "agent-runtime-promotion-proposal",
        "proposal kind",
    )
    ...
    if set(agent_bundle) != {"prompt_key", "persona_key", "tool_names"}:
        raise PromotionReviewError("proposal agent bundle is incomplete")
    if agent_bundle.get("tool_names") != ["search_memory"]:
        raise PromotionReviewError("proposal agent bundle must use only search_memory")
    for key in ("prompt_key", "persona_key"):
        value = agent_bundle.get(key)
        if not isinstance(value, str) or not value.strip():
            raise PromotionReviewError(f"proposal agent bundle {key} is invalid")
        _require_equal(qualification_config.get(key), value, f"agent bundle {key}")
    ...
    if set(effective) != set(qualification_config) | {
        "api_base_url",
        "output_dir",
        "llama_compatibility_model_key",
        "include_llama_compatibility",
    }:
        raise PromotionReviewError("effective config keys are incomplete")
