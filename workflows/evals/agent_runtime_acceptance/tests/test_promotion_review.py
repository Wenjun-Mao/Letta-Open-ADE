...
from workflows.evals.agent_runtime_acceptance.policy import production_policy_hashes
...
def test_review_rejects_agent_bundle_that_differs_from_qualification_config(
    tmp_path: Path,
) -> None:
    manifest_path = tmp_path / "deployment-manifest.json"
    manifest_path.write_bytes(
    ...
    proposal["agent_bundle"]["persona_key"] = "chat_other_persona"
    proposal["proposal_sha256"] = hashlib.sha256(_canonical_bytes(proposal)).hexdigest()
    proposal_path.write_bytes(_canonical_bytes(proposal))

    with pytest.raises(PromotionReviewError, match="agent bundle persona_key"):
        review_promotion(
            proposal_path=proposal_path,
            manifest_path=manifest_path,
            project_root=PROJECT_ROOT,
...
def test_review_rejects_effective_config_that_differs_from_proposal(
    tmp_path: Path,
) -> None:
    manifest_path = tmp_path / "deployment-manifest.json"
    manifest_path.write_bytes(
    ...
    proposal_path = _evidence(tmp_path, manifest_path)
    provenance_path = proposal_path.parent / "provenance.json"
    provenance = json.loads(provenance_path.read_text(encoding="utf-8"))
    provenance.pop("provenance_sha256")
    provenance["effective_config"]["prompt_key"] = "chat_other_prompt"
    ...
    proposal = json.loads(proposal_path.read_text(encoding="utf-8"))
    proposal.pop("proposal_sha256")
    proposal["provenance_sha256"] = provenance["provenance_sha256"]
    proposal["proposal_sha256"] = hashlib.sha256(_canonical_bytes(proposal)).hexdigest()
    proposal_path.write_bytes(_canonical_bytes(proposal))

    with pytest.raises(PromotionReviewError, match="effective config prompt_key"):
        review_promotion(
            proposal_path=proposal_path,
            manifest_path=manifest_path,
            project_root=PROJECT_ROOT,
    ...
    return (
        {
            ...
            "resources": {
                "conversation_ids": [f"conversation-{case.key}"],
            },
        },
        raw_events,
    )
