...
class CaseExecution:
    ...
    resources: EvaluationSessionScope
...
async def execute_case(
    *,
    ...
    for conversation_key, binding in dict(getattr(case, "conversations")).items():
        ...
        session = await _create_session(
            ...
            idempotency_key=_resource_key(
                namespace, str(getattr(case, "key")), f"conversation-{conversation_key}"
            ),
            title=f"Acceptance {getattr(case, 'key')} {conversation_key}",
            model_key=None if agent_key in definitions else conversation_model_key,
            reviewer_model_key=(
                None if agent_key in definitions else reviewer_model_key
            ),
            embedding_model_key=(
                None if agent_key in definitions else embedding_model_key
            ),
            prompt_key=None if agent_key in definitions else prompt_key,
            persona_key=None if agent_key in definitions else persona_key,
            tool_names=None if agent_key in definitions else definition_tools,
            agent_definition_id=definitions.get(agent_key),
            memory_subject_id=subjects.get(subject_key),
            ...
            subject_display_name=(
                None if subject_key in subjects else f"Acceptance {subject_key}"
            ),
        )
        definition = _nested_object(session, "agent_definition")
        subject = _nested_object(session, "memory_subject")
        conversation = _nested_object(session, "conversation")
        definition_id = _id(definition, "agent definition")
        subject_id = _id(subject, "memory subject")
        conversation_id = _id(conversation, "conversation")
        definitions.setdefault(agent_key, definition_id)
        subjects.setdefault(subject_key, subject_id)
        conversations[str(conversation_key)] = conversation_id
        _record_session_scope(
            session_ids,
            session_scope_sink,
    ...
    # Evaluation sessions do not expose a mutable seed endpoint. Seed facts are
    # normal turns in an auxiliary no-tool session bound to the same subject.
    ...
    for subject_key, subject_facts in initial_facts_by_subject.items():
        setup_session = await _create_session(
            client=client,
            idempotency_key=_resource_key(
                namespace, str(getattr(case, "key")), f"initial-facts-{subject_key}"
            ),
            title=f"Acceptance {getattr(case, 'key')} initial facts {subject_key}",
            ...
            tool_names=(),
            memory_subject_id=subjects[subject_key],
        )
        setup_definition = _nested_object(setup_session, "agent_definition")
        setup_conversation_id = _id(
            _nested_object(setup_session, "conversation"), "conversation"
        )
        _record_session_scope(
            session_ids,
            session_scope_sink,
        ...
        response = await _run_stage(
            ...
            client.get_evaluation_session_state(setup_conversation_id),
        )
        for fact in subject_facts:
            _verify_public_initial_fact(
                response=_nested_object(response, "memories"),
    ...
    for index, turn in enumerate(tuple(getattr(case, "turns")), start=1):
        ...
        state = await _run_stage(
            ...
            client.get_evaluation_session_state(conversation_id),
        )
    ...
    for subject_key, subject_id in subjects.items():
        conversation_id = next(
            conversation_id
            for conversation_key, conversation_id in conversations.items()
            if str(dict(getattr(case, "conversations"))[conversation_key][1])
            == subject_key
        )
        response = await _run_stage(
            "final_memory_observation",
            client.get_evaluation_session_state(conversation_id),
        )
        values = _nested_object(response, "memories").get("facts")
...
async def _create_session(
    *,
    ...
    subject_external_key: str | None = None,
    subject_display_name: str | None = None,
) -> dict[str, Any]:
    return await _run_stage(
        "evaluation_session_setup",
        client.create_evaluation_session(
...
def _nested_object(payload: dict[str, Any], key: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"evaluation session response is missing {key}")
    return value


def _record_session_scope(
    session_ids: list[str],
    sink: list[EvaluationSessionScope] | None,
    *,
    conversation_id: str,
    ...
    deployment_fingerprints = _deployment_fingerprints(definition)
    snapshots = _deployment_snapshots(definition)
    fingerprints.update(deployment_fingerprints)
    deployment_snapshots.extend(snapshots)
    if sink is not None:
        sink.append(
            EvaluationSessionScope(
                conversation_ids=(conversation_id,),
                deployment_fingerprints=deployment_fingerprints,
                deployment_snapshots=snapshots,
            )
        )
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "accepted runtime run could not be cancelled before cleanup"
            ) from cleanup_error
        raise


async def _cancel_and_await_terminal(
    client: RuntimeClient, accepted: dict[str, Any]
) -> tuple[dict[str, Any], tuple[Any, ...]]:
    ...
    try:
        ...
        if run.get("status") not in {"succeeded", "failed", "cancelled"}:
            raise RuntimeError(
                f"cancelled runtime run {run_id} did not terminate"
            ) from exc
    return run, events
...
def _failed_case_execution(
    case: object,
    exc: Exception,
    scopes: list[EvaluationSessionScope],
...
def _combined_scope_fingerprints(
    scopes: list[EvaluationSessionScope],
) -> dict[str, str]:
    combined: dict[str, str] = {}
    for scope in scopes:
...
def _id(payload: dict[str, Any], kind: str) -> str:
    ...
    if not value:
        raise RuntimeError(f"Agent Runtime {kind} response is missing id")
    return value
