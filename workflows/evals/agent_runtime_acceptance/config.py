...
QUALIFIED_AGENT_TOOL_NAMES = ("search_memory",)
...
class AcceptanceConfig:
    ...
    def validate(self) -> None:
        ...
        if not self.prompt_key.strip():
            raise ConfigError("prompt_key is required")
        if not self.persona_key.strip():
            raise ConfigError("persona_key is required")
    ...
    config = AcceptanceConfig(
        ...
        output_dir=_project_path(
            _value(
                ...
                payload,
                "output_dir",
                "workflows/evals/agent_runtime_acceptance/outputs",
            )
        ),
        ...
        prompt_key=_value("PROMPT_KEY", payload, "prompt_key", DEFAULT_PROMPT_KEY),
        persona_key=_value("PERSONA_KEY", payload, "persona_key", DEFAULT_PERSONA_KEY),
...
def _value(env_key: str, payload: dict[str, Any], key: str, default: str) -> str:
    return str(
        os.getenv(f"AGENT_RUNTIME_ACCEPTANCE_{env_key}") or payload.get(key) or default
    ).strip()


def _int_value(env_key: str, payload: dict[str, Any], key: str, default: int) -> int:
    value = os.getenv(f"AGENT_RUNTIME_ACCEPTANCE_{env_key}")
...
def _case_keys_value(payload: dict[str, Any]) -> tuple[str, ...]:
    configured = os.getenv("AGENT_RUNTIME_ACCEPTANCE_CASE_KEYS")
