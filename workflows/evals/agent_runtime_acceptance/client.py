...
class ApiResponseError(RuntimeClientError):
    def __init__(self, status_code: int, code: str, message: str) -> None:
        super().__init__(f"Agent Runtime API {status_code} {code}: {message}")
...
class RuntimeClient:
    """Authenticated client for evaluation sessions and normalized run events."""
    ...
    async def create_evaluation_session(
        ...
        *,
        ...
        """Create one evaluation-owned conversation or bind shared fixtures.

        Optional fields are omitted instead of sent as null so the API can tell
        a reused definition or subject from a request to configure a new one.
        """
        ...
        return await self._request_json("POST", "/api/v3/evaluation-sessions", payload)
    ...
    async def get_evaluation_session_state(
        ...
        return await self._request_json(
            "GET", f"/api/v3/evaluation-sessions/{conversation_id}/state"
        )
        ...
        if candidate.scheme or candidate.netloc:
            if (candidate.scheme, candidate.netloc) != (base.scheme, base.netloc):
                raise RuntimeClientError(
                    "Agent Runtime API URL must remain on the configured API origin"
                )
            return value
        if not value.startswith("/"):
            raise RuntimeClientError(
                "Agent Runtime API URL must be absolute-path relative"
            )
...
class _SseParser:
    ...
    def _emit(self) -> SseEvent | None:
        ...
        try:
            ...
            raise RuntimeClientError(
                "runtime SSE event contains malformed JSON"
            ) from exc
        if not isinstance(data, dict):
            raise RuntimeClientError("runtime SSE event payload must be an object")
...
def _required_string(payload: dict[str, Any], key: str) -> str:
    ...
    if not value:
        raise RuntimeClientError(f"runtime API response is missing {key}")
    return value
...
def _response_json_object(response: httpx.Response) -> dict[str, Any]:
    try:
        ...
        raise RuntimeClientError("runtime API returned malformed JSON") from exc
    if not isinstance(data, dict):
        raise RuntimeClientError("runtime API returned a non-object JSON response")
    return data
