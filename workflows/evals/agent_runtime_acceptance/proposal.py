...
def build_promotion_proposal(
    *,
    ...
    if not _is_full_matrix_qualification_config(qualification_config):
        return None
    if not _is_qualified_agent_bundle(agent_bundle, qualification_config):
        return None
    ...
    material = {
        "schema_version": 1,
        "kind": "agent-runtime-promotion-proposal",
        "run_id": run_id,
...
def _is_qualified_agent_bundle(
    bundle: dict[str, Any], qualification_config: dict[str, Any]
) -> bool:
    return (
        set(bundle) == {"prompt_key", "persona_key", "tool_names"}
        and isinstance(bundle.get("prompt_key"), str)
        and bool(bundle["prompt_key"].strip())
        and bundle.get("prompt_key") == qualification_config.get("prompt_key")
        and isinstance(bundle.get("persona_key"), str)
        and bool(bundle["persona_key"].strip())
        and bundle.get("persona_key") == qualification_config.get("persona_key")
        and bundle.get("tool_names") == ["search_memory"]
    )
