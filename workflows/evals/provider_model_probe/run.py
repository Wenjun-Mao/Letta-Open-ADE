...
from model_catalog_contracts.model_allowlist import resolve_source_allowlist_path
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source-id",
        required=True,
        help="Configured source id from config/model-router/sources.json.",
    )
    parser.add_argument(
        "--mode",
        default="chat-probe",
    ...
    parser.add_argument(
        "--write",
        action="store_true",
        help="Write the probe report to the configured allowlist path.",
    )
    parser.add_argument(
        "--output", help="Optional output path override for the generated report."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_path = _resolve_output_path(
        source_id, probe_mode=args.mode, explicit_path=args.output
    )
...
def _resolve_output_path(
    source_id: str, *, probe_mode: str, explicit_path: str | None
) -> Path:
    if explicit_path:
