...
from workflows.evals.provider_model_probe.label_fixture import (
    LABEL_PROBE_ARTICLE,
...
def probe_source_chat_models(
    ...
    *,
    ...
    usable_models = tuple(
        result.provider_model_id for result in results if result.usable
    )
    ...
    *,
    ...
    retrying = Retrying(
        ...
        retry=retry_if_exception_type(
            (RetryableProbeError, *_RETRYABLE_PROBE_EXCEPTIONS)
        ),
        reraise=True,
    )
...
def _post_chat_probe_with_retries(
    ...
    *,
    ...
    for attempt in retrying:
        with attempt:
            return _post_chat_probe_once(
                source, model_id=model_id, timeout_seconds=timeout_seconds
            )
    ...
    *,
    ...
    with httpx.Client(timeout=timeout_seconds) as session:
        response = session.post(
            source.chat_completions_url(), headers=headers, json=payload
        )
    ...
    *,
    ...
    payload = {
        ...
        "response_format": label_response_format(
            label_probe_output_schema(), name="label_probe_output"
        ),
    }
