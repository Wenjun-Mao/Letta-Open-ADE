...
from workflows.evals.provider_model_probe.contracts import (
    ProbeCatalogAuthError,
    RetryableProbeError,
    SourceProbeReport,
)
from workflows.evals.provider_model_probe.classifiers import (
    classify_chat_probe_payload,
)
from workflows.evals.provider_model_probe.probe import (
    probe_chat_model,
    probe_source_chat_models,
)
import workflows.evals.provider_model_probe.probe as provider_model_probe
...
def test_classify_chat_probe_payload_maps_non_success_cases(
    payload, expected_status: str
) -> None:
    result = classify_chat_probe_payload(
...
def test_probe_chat_model_maps_retry_and_timeout_errors(
    monkeypatch, exc: Exception, expected_status: str
) -> None:
    monkeypatch.setattr(
...
def test_probe_source_chat_models_skips_non_llm_entries(monkeypatch) -> None:
    monkeypatch.setattr(
        ...
        lambda source, *, timeout_seconds: [
            RouterModelRecord(
                provider_model_id="doubao-seed-1-8-251228", model_type="llm"
            ),
            RouterModelRecord(
                provider_model_id="doubao-embedding-text-240715", model_type="embedding"
            ),
        ],
    )
    monkeypatch.setattr(
