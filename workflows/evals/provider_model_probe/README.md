...
```bash
uv run python workflows/evals/provider_model_probe/run.py --source-id ark --mode chat-probe
```
...
```bash
uv run python workflows/evals/provider_model_probe/run.py --source-id ark --mode label-structured --write
```
...
| Argument | Meaning |
| --- | --- |
| `--source-id` | Required router source id from `config/model-router/sources.json`. |
