...
from workflows.evals.comment_persona_eval.artifacts import (
    ArtifactWriter,
    write_artifacts,
)
...
def _response(
    status_code: int, payload: dict[str, Any], request: httpx.Request
) -> httpx.Response:
    ...
    with httpx.Client(
        ...
        try:
            ...
            raise AssertionError(
                "Expected unavailable model_key to fail preflight validation"
            )
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return _response(
            200,
            {
                ...
                "raw_reply": {
                    "timings": {"cache_n": 0, "prompt_n": 123, "predicted_n": 24}
                },
            },
            request,
        )
...
def test_write_artifacts_preserves_csv_and_raw_jsonl(tmp_path) -> None:
    ...
    write_artifacts(
        csv_path=csv_path, jsonl_path=jsonl_path, rows=rows, raw_records=raw_records
    )
...
def test_artifact_writer_streams_attempts_before_context_closes(tmp_path) -> None:
    ...
    with ArtifactWriter(csv_path=csv_path, jsonl_path=jsonl_path) as writer:
        ...
        assert (
            json.loads(jsonl_path.read_text(encoding="utf-8").strip())["row_id"]
            == "run-1::1::comment_demo"
        )
