...
export function listTestRuns(options?: ApiRequestOptions) {
  return requestJson<{ items: PlatformRunRecord[] }>("/api/v2/test-center/runs", options);
}

export function createTestRun(payload: CreateTestRunPayload) {
  return requestJson<PlatformRunRecord>("/api/v2/test-center/runs", { method: "POST", body: payload });
}

export function getTestRun(runId: string, options?: ApiRequestOptions) {
  return requestJson<PlatformRunRecord>(`/api/v2/test-center/runs/${runId}`, options);
}

export function cancelTestRun(runId: string) {
  return requestJson<PlatformRunRecord>(`/api/v2/test-center/runs/${runId}/cancel`, { method: "POST" });
}

export function listRunArtifacts(runId: string, options?: ApiRequestOptions) {
  return requestJson<{ run_id: string; items: PlatformArtifact[] }>(`/api/v2/test-center/runs/${runId}/artifacts`, options);
}
