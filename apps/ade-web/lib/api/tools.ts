...
export function listTools(search = "", limit = 200, agentId = "", options?: ApiRequestOptions) {
  ...
  return requestJson<{ total: number; items: PlatformTool[] }>(`/api/v2/tool-center/runtime-tools?${params.toString()}`, options);
}
...
export function testInvokeTool(payload: {
  ...
  return requestJson<PlatformToolTestInvokeResult>("/api/v2/tool-center/invocations", { method: "POST", body, signal });
}
