...
function Conversation({ controller, t }: { controller: Controller; t: Translate }) {
  ...
  if (!controller.session || !controller.conversation) {
    return <section className="card studio-empty-state"><div className="kicker">{t("ADE-native runtime", "ADE 原生运行时")}</div><h2>{t("Definitions first, then memory subjects", "先定义，再选择记忆主体")}</h2><p>{t("Create a reusable immutable definition version, choose an explicit person or organization to own memory, then start a conversation. This keeps memory shared only where you deliberately bind the same subject.", "创建可复用的不可变定义版本，选择拥有记忆的明确个人或组织，然后开始对话。只有在明确绑定同一主体时，记忆才会被共享。")}</p></section>;
  }
