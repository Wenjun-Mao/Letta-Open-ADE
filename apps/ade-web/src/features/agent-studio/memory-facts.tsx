...
function FactRow({ fact, t, openEvidence, prepareAction, removeSaved, canCorrect, canRemove }: {
  ...
  return <details><summary><span><strong>{fact.fact_type}</strong> · {fact.entity_label || fact.entity_kind}</span><span className={fact.status === "active" ? "studio-status studio-status-good" : "studio-status studio-status-bad"}>{fact.status} · v{fact.version}</span></summary>
    ...
    {canRemove && fact.status !== "forgotten" && removeSaved ? confirming ? <div className="studio-removal-confirmation">
      <p>{t("Remove exactly", "确认移除")} <strong>{fact.fact_type} v{fact.version}: {fact.value || t("former assertion", "过往陈述")}</strong>?</p>
      <p className="muted">{t("This excludes this fact chain from model fact selection; past messages, summaries, and audit revisions remain.", "这会将该事实链排除出模型事实选择；过往消息、摘要及审计修订仍会保留。")}</p>
      <div className="toolbar">
        <button className="button muted" onClick={() => setConfirmingVersion(null)}>{t("Cancel removal", "取消移除")}</button>
