...
import contract from "../../../../../config/agent-studio/memory-action-contract.json";
...
const action: PendingMemoryAction = { conversationId: "conversation-1", factId: "fact-1", version: 2, operation: "forget", runId: "run-1", outcome: "Waiting for review." };
...
describe("reviewed memory action status", () => {
  it("keeps the composer draft aligned with the backend review-intent contract", () => {
    const fact = contract.fact as MemoryFact;
    expect(memoryActionDraft(fact, "correct")).toBe(contract.correction_draft);
    expect(`${memoryActionDraft(fact, "correct")}green tea`).toBe(contract.correction_message);
    expect(memoryActionDraft(fact, "forget")).toBe(contract.removal_draft);
  });
