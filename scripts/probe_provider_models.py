...
from utils.provider_model_probe import probe_source_chat_models
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="chat-probe",
        choices=["chat-probe"],
        help="Probe mode to run. Only chat-probe is currently supported.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_path = _resolve_output_path(source_id, explicit_path=args.output)
...
def _resolve_output_path(source_id: str, *, explicit_path: str | None) -> Path:
    if explicit_path:
        return Path(explicit_path).expanduser().resolve()

    configured_path = resolve_source_allowlist_path(source_id)
    if configured_path is None:
        raise ValueError(f"No checked-in allowlist path is configured for source '{source_id}'")
