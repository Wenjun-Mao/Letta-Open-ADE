...
def _registry(args: argparse.Namespace) -> PersonaSqliteRegistry:
    ...
    return PersonaSqliteRegistry(
        project_root, db_path=db_path, seed_jsonl_path=seed_path
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Import/export ADE SQLite persona library data."
    )
    parser.add_argument("--db-path", default="data/runtime/personas/personas.sqlite3")
    parser.add_argument("--seed-jsonl", default="content/personas/personas.jsonl")
    subparsers = parser.add_subparsers(dest="command", required=True)

    import_parser = subparsers.add_parser(
        "import-jsonl", help="Import personas from JSONL."
    )
    import_parser.add_argument("--input", required=True)
    import_parser.add_argument(
        "--on-conflict", choices=["error", "skip", "upsert"], default="error"
    )

    export_parser = subparsers.add_parser(
        "export-jsonl", help="Export personas to JSONL."
    )
    ...
    markdown_parser = subparsers.add_parser(
        "export-markdown", help="Export personas to Markdown."
    )
    markdown_parser.add_argument("--output", required=True)
    markdown_parser.add_argument(
        "--scenario", choices=["chat", "comment"], default=None
    )
    markdown_parser.add_argument("--include-archived", action="store_true")

    subparsers.add_parser(
        "sync-seed",
        help="Synchronize the reviewed JSONL seed into the runtime database.",
    )
