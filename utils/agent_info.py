...
def pretty_print_messages(messages):
    """
    Parses and beautifully prints the dense array of messages returned by Letta.
    Differentiates clearly between System info, Internal Reasoning, Tool Calls, and User-facing text.
    """
    print("\n" + "="*50)
    for msg in messages:
        ...
        # Agent's direct response to the user
        elif m_type == "assistant_message":
            content = getattr(msg, "content", "")
            print(f"🗣️ ASSISTANT (Lin Xiao Tang):\n  {content.strip()}")
            
        # Agent calling a Letta Tool (like core_memory_append)
        ...
        # Tool Returning data back to the agent
        elif m_type == "tool_return_message":
            status = getattr(msg, "status", "")
            content = getattr(msg, "tool_return", getattr(msg, "content", ""))
            print(f"✅ TOOL RETURN [{status}]: {content}")
            
        # System instructions
        elif m_type == "system_message":
            pass # Usually hidden unless we want to see it, it's too long
        
        else:
            # Fallback for anything else
            print(f"❓ [{m_type.upper()}]: {msg}")
            
        # Don't print separator for system messages that we skipeed
        if m_type != "system_message":
            print("-" * 50)
