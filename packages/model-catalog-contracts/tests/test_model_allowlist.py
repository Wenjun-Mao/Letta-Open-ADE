...
import model_catalog_contracts.model_allowlist as model_allowlist


def test_load_configured_source_allowlist_parses_valid_report(
    ...
    assert result.usable_models == frozenset(
        {"doubao-seed-1-8-251228", "glm-4-7-251222"}
    )


def test_load_configured_source_allowlist_fails_closed_for_invalid_payload(
    monkeypatch, tmp_path
) -> None:
    report_path = tmp_path / "ark_chat_probe_report.json"
